  ...
  div.con-box
    div.wrap
      div.box-header.row
        div.col-md-12
          div.col-md-9
            nuxt-link.btn.btn-success(to="/producttemp/new" role="button")
              i.fe.fe-plus.mr-2
              | 新增
          div.col-md-3
          ...
          table.table.card-table.table-striped.table-vcenter
            ...
            tbody
              tr(v-for="(item, num) in dataArr")
                ...
                td 
                  span.switch-plug(:data-content="item.state == 'on' ? '启用' : '禁用'" :data-switch="item.state" @click="switchStateBtn('table_product_templates', item, 'state')")
                td.opers
                  nuxt-link(:to="'/producttemp/edit?id=' + item.id" title="编辑" )
                    i.fa.fa-pencil
                  a(href="javascript:;" title="删除" @click="deleteListItem('table_product_templates', item, dataArr)")
                    i.fa.fa-trash
    ...
    methods: {
      ...
      getListData: async  function (obj) {
        ...
        if (_.isEmpty(model.basicdata.searchkey)) {
          ...
          result = await API.GET('classes/table_product_templates', condition)
        } else {
          // 搜索
          let searchpar = {
            table: 'table_product_templates',
