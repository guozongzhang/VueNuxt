<template lang="pug">
 div.con-box
    div.nav-self-box
      <crumbs-vue obj="materialtype"></crumbs-vue>
      div.info-body
        div.wrap
          div.box-header.row
            div.col-md-12
              div.col-md-9
                nuxt-link.btn.btn-success(to="/goodsclassify/newmaterial"  role="button")
                  i.fe.fe-plus.mr-2
                  | 新增
              div.col-md-3
                <searchbasic flag="materiallistsearch" :obj="basicdata" @getsearchdate="getListData"></searchbasic>
          div.body-box
              ...
              table.table.card-table.table-striped.table-vcenter
                ...
                tbody
                  tr(v-for="(item, num) in dataArr")
                    ...
                    td.opers
                      nuxt-link(:to="'/goodsclassify/editmaterial?id=' + item.id" title="编辑" )
                        i.fa.fa-pencil
                      a(href="javascript:;" title="删除" @click="deleteListItem('furniture_materials', item, dataArr)")
                        i.fa.fa-trash
            div.loading-box(v-if="loadingState")
              vueloading
            pagination(flag="materiallist" v-bind:total="basicdata.pagetotal" v-bind:size="basicdata.pagesize" v-on:callback="getListData")
</template>
    ...
    methods: {
      ...
      getListData: async  function (obj) {
        ...
        if (_.isEmpty(model.basicdata.searchkey)) {
          ...
          result = await API.GET('classes/furniture_materials', condition)
        } else {
          // 搜索
          let searchpar = {
            table: 'furniture_materials',
            like: JSON.stringify(['mat_name']),
