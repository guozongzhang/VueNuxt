<template lang="pug">
 div.con-box
    div.nav-self-box
      <crumbs-vue obj="styletype"></crumbs-vue>
      div.info-body
        div.wrap
          div.box-header.row
            div.col-md-12
              div.col-md-9
                nuxt-link.btn.btn-success(to="/goodsclassify/newstyle"  role="button")
                  i.fe.fe-plus.mr-2
                  | 新增
              div.col-md-3
                <searchbasic flag="stylelistsearch" :obj="basicdata" @getsearchdate="getListData"></searchbasic>
          div.body-box
              ...
              table.table.card-table.table-striped.table-vcenter
                ...
                tbody
                  tr(v-for="(item, num) in dataArr")
                    ...
                    td.opers
                      nuxt-link(:to="'/goodsclassify/editstyle?id=' + item.id" title="编辑" )
                        i.fa.fa-pencil
                      a(href="javascript:;" title="删除" @click="deleteListItem('furniture_styles', item, dataArr)")
                        i.fa.fa-trash
            div.loading-box(v-if="loadingState")
              vueloading
            pagination(flag="stylelist" v-bind:total="basicdata.pagetotal" v-bind:size="basicdata.pagesize" v-on:callback="getListData")
</template>
    ...
    methods: {
      ...
      getListData: async  function (obj) {
        ...
        if (_.isEmpty(model.basicdata.searchkey)) {
          ...
          result = await API.GET('classes/furniture_styles', condition)
        } else {
          // 搜索
          let searchpar = {
            table: 'furniture_styles',
            like: JSON.stringify(['style_name']),
