  ...
  div.nav-self-box
    ...
    div.info-body
      div.box-header
        div.col-md-9
        div.col-md-3
          <searchbasic flag="deletedmailsearch" :obj="basicdata" @getsearchdate="getListData"></searchbasic>        
        div.body-box
          ...
          div.loading-box(v-if="loadingState")
            vueloading
          pagination(flag="deletedmaillist" v-bind:total="basicdata.pagetotal" v-bind:size="basicdata.pagesize" v-on:callback="getListData")
</template>
<script>
  let model
    ...
    methods: {
      ...
      getListData: async function (obj) {
        ...
        if (_.isEmpty(model.basicdata.searchkey)) {
          ...
          result = await API.GET('functions/messages/messages_inmail_has_delete')
        } else {
          //  搜索
          let searchpar = {
            table: 'products',
