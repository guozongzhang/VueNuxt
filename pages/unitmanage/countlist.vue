  ...
  div.con-box
    div.wrap
      div.box-header.row
        div.col-md-12
          div.col-md-9
            nuxt-link.btn.btn-success(to="/unitmanage/newcount"  role="button")
              i.fe.fe-plus.mr-2
              | 新增
          div.col-md-3
      ...
      div.body-box
        div.table-responsive
          table.table.card-table.table-striped.table-vcenter
            ...
            tbody
              tr(v-for="(item, num) in countArr")
                td {{num + 1}}
                td 
                  nuxt-link(:to="'/unitmanage/indexcount?id=' + item.id") {{item.name}}
                td {{item.remark || '暂无'}}
                td {{(item.uid_poi_admin || {}).username}}
                td {{localDate(item.update_time)}}
                td 
                  span.switch-plug(:data-content="item.switch_state == 'on' ? '启用' : '禁用'" :data-switch="item.switch_state" @click="switchStateBtn('unit_count', item, 'switch_state')")
                td.opers
                  nuxt-link(:to="'/unitmanage/editcount?id=' + item.id" title="编辑" )
                    i.fa.fa-pencil
                  a(href="javascript:;" title="删除" @click="deleteListItem('unit_count', item, countArr)")
                    i.fa.fa-trash
    ...
    methods: {
      ...
      getListData: async  function (obj) {
        ...
        if (_.isEmpty(model.basicdata.searchkey)) {
          ...
          result = await API.GET('classes/unit_count', condition)
        } else {
          // 搜索
          let searchpar = {
            table: 'unit_count',
        ...
        model.countArr = result.data.items
