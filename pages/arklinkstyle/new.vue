          ...
          div.form-group
            div.input-group
              span.input-group-prepend
                span.input-group-text 
                  | 安装个数：
              select.form-control(v-model="info.setup_count" @change="changeNumber(info)")
                option(value="-1") 请选择
  ...
  export default {
    ...
    methods: {
      ...
      changeType: function (obj) {
        obj.subArr = []
        obj.type_poi_hardware_third_parts_type = -1
        model.hardThirdTypeArr.forEach((sub) => {
      ...
      commitBtn: function () {
        ...
        API.POST('functions/connection/technology_connection', model.info).then((msg) => {
